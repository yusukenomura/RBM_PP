...
program RBM_solver 
  ...
  real(8) , allocatable :: Chi_sp (:,:)  ! spin correlation function 
  real(8) , allocatable :: Chi_vbs(:,:)  ! correlation function for VBS 
...
!! DELETE delete Delete
  ...
  allocate( Chi_sp (2,Nchi) ); Chi_sp  = 0d0
  allocate( Chi_vbs(Nchi,4) ); Chi_vbs = 0d0
...
goto 1111
  ...
  do iteration = 1, Nstep 
    ...
    wf_av = wf_av / 10d0
    !shift = wf_av * 0.0005d0 / dble(N) / dble(N)
    shift = wf_av*0.00001d0
...
1111 continue
  ...
  do isample = 1, Nsample
    wf_av  = wf_av + abs(psi_x_sample(0,isample))
  end do ! isample
  ...
  shift = wf_av * 1d-5
  call MPI_BCAST(shift,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
  if( mod(myrank,100) == 0 ) write(6,'(a,I7,2E15.5)') 'myrank, wf_av, shift =', myrank, wf_av, shift
  ...
  do iteration = 1, Nav 
    ...
    ! 
    call system_clock(t1)
    call calc_phys_quantity_after_opt(Etot,Etot2,Chi_sp,Chi_vbs,norm,wf_av,wf_sgn)
    call measure_time(t1,t(3))
    ! 
    ! gather data
    ! 
    ...
    call MPI_REDUCE(wf_sgn,c1,1,MPI_DOUBLE_COMPLEX,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if( myrank == 0 ) wf_sgn = c1 / dble(Nsample*Nmpi) 

    allocate( Chi_mpi(2,Nchi) ); Chi_mpi = 0d0
    call MPI_REDUCE(Chi_sp,Chi_mpi,2*Nchi,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if( myrank == 0 ) Chi_sp(:,:) = Chi_mpi(:,:) / norm!dble(Nsample*Nmpi)
    deallocate(Chi_mpi)

    call MPI_BARRIER(MPI_COMM_WORLD,ierr) 
    allocate( Chi_mpi(Nchi,4) ); Chi_mpi = 0d0
    call MPI_REDUCE(Chi_vbs,Chi_mpi,4*Nchi,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if( myrank == 0 ) Chi_vbs(:,:) = Chi_mpi(:,:) / norm!dble(Nsample*Nmpi)
    deallocate(Chi_mpi)
    call measure_time(t1,t(5))
    ! 
    ! write info
    ...
    if( myrank == 0 ) then 
      ...
      write(6,'(a,2E20.5)')     '   wf_av, shift         :', wf_av, shift
      write(6,'(a,F20.10)')     '   norm                 :', norm/dble(Nsample*Nmpi)
      write(6,'(a,3F20.10)')    '   wf_sgn               :', abs(wf_sgn), wf_sgn
      write(6,'(a,6F16.8)')     '   computational time   :', t(:)
      Etot_data (iteration) = Etot
      Etot2_data(iteration) = Etot2-Etot*Etot
      rewind(60000+iteration)
      write(60000+iteration,'(a)') '# chi index,  SzSz,  SpSm'
      do i = 1, Nchi 
        write(60000+iteration,'(I6,2F25.15)') i, Chi_sp(1,i), Chi_sp(2,i) 
      end do ! i 
      close(60000+iteration)
      rewind(70000+iteration)
      write(70000+iteration,'(a)') '# chi index, <SSSS> for (1,0), (0,1), (1,1), (1,-1)'
      do i = 1, Nchi 
        write(70000+iteration,'(I6,4F25.15)') i, (Chi_vbs(i,j), j = 1, 4)  
      end do ! i 
      close(70000+iteration)
      write(10,*) iteration, Etot, (Etot2-Etot*Etot)/(Etot*Etot)
    end if 
