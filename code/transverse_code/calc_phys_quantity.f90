...
subroutine calc_phys_quantity_opt(myrank,Etot,Etot2,gvec,Ovec,Smat,norm,wf_av)
  ...
  real(8)    :: Ovec_Store(Nv,Nsample*2)  
  ...
  do isample = 1, Nsample
    ...
    call calc_local_energy_correlation(x_sample(:,isample),psi_x(0),thetac,thetar,& 
                                       Ele_Idx,Ele_Cfg,sgn_PP,Pf_M,Inv_M,Eloc_x)
    ...
    do k = 1, Nv
      ...
      Ovec_Store(k,2*isample-1) = dble (Ovec_loc_x(k))  
      Ovec_Store(k,2*isample  ) = dimag(Ovec_loc_x(k))  
    end do ! k 
    !$omp end parallel do 
  ...
  call system_clock(t1)
  call dgemm('N','T',Nv,Nv,Nsample*2,1d0,Ovec_Store,Nv,Ovec_Store,Nv,0d0,Smat,Nv)
  call measure_time(t1,t(10))

  if( mod(myrank,100) == 0 ) write(50000+myrank,'(10F10.6)') t(:)
...
!
!
subroutine calc_phys_quantity_after_opt(Etot,Etot2,norm,wf_av)
  use mod_main   , only : N, N_QP_Full, N_MP_Trans, shift
...
! 
! 
subroutine calc_local_energy_correlation(x,psi_x,thetac,thetar,Ele_Idx,Ele_Cfg,sgn_PP,Pf_M,Inv_M,Eloc_x)
